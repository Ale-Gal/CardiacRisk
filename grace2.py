def grace(patient_data, risk_factors, risk_threshold=145,debug=False):
    ...
    for i in range(0, len(risk_factors)):
        ...
        for j in range(13): 
         ...
         if factor == "Age":
            age = patient_data[1][j]   # (patient_data['Age'][j]) e non (patient_data[i])   
            ...
            creatinine = patient_data['Creat'][j]
            ...
            if patient_data['CAA'][j] == 1:
                risk += 98
            
         elif factor == "TN":
            if patient_data['TN'][j] == 1:
                risk += 54
            
         elif factor == "DEP ST":
            if patient_data['DEP ST'][j] == 1:
                risk += 67
