def grace1(patient_data, risk_factors, risk_threshold=145, debug=False):  
    ...
    if risk > 0 and risk < 145: 
        p= (0.5*risk)/145 #caso tra 0 e 145: basso rischio=false 145:50=risk:p 
        pp=float("{:.2f}".format(p))
    elif risk >= 145:
        p= (0.5*risk)/(528-145) #caso tra 145 e 528: alto rischio=true (528-145):50=risk:p
        pp=float("{:.2f}".format(p))
    return pp
